<section class="background-w-imageaventon text-white mb-0 row" style="background-image: url('{{ asset('assets/autos.jpg') }}')" id="#ahorra">
  <div class="container">
    <h2 class="text-center text-uppercase text-white">Ahorrá y ayudá </h2>
    @include('flash_message')
    <div class="row">
      <div class="col-lg-4 offset-4">
        <a class="btn btn-xl btn-outline-light text-center color-aventon" href="/search">
          Buscar viajes
        </a>
      </div>
      
      <div class="col-lg-4 offset-4">
        <a class="btn btn-xl btn-outline-light text-center color-aventon" href="/ride/create">
          Publicar viaje
        </a>
      </div>
    <div>
  </div>
</section>